import type { EnvOverrideCatalogItem } from "@/lib/codex/types";

function normalizeEnvKey(value: unknown) {
  return String(value || "").trim().toUpperCase();
}

function normalizeEnvTextValue(value: unknown) {
  return String(value ?? "").trim();
}

function fallbackEnvLabel(key: string) {
  return key || "未命名变量";
}

const ENV_OVERRIDE_DESCRIPTION_MAP: Record<string, string> = {
  CODEXMANAGER_UPSTREAM_TOTAL_TIMEOUT_MS:
    "控制单次上游请求允许持续的最长时间，单位毫秒；超过后会主动结束请求并返回超时错误。",
  CODEXMANAGER_UPSTREAM_STREAM_TIMEOUT_MS:
    "控制流式上游请求允许持续的最长时间，单位毫秒；填 0 可关闭流式超时上限，适合长时间持续输出的 SSE/流式连接。",
...
export function buildEnvOverrideOptionLabel(
  item: Pick<EnvOverrideCatalogItem, "key" | "name"> | null | undefined,
) {
  if (!item) {
    return "";
  }
  return String(item.name || "").trim() || fallbackEnvLabel(item.key);
}

export function buildEnvOverrideDescription(
  item: Pick<EnvOverrideCatalogItem, "key" | "name" | "description"> | null | undefined,
) {
  if (!item) {
  ...
  const label = buildEnvOverrideOptionLabel(item);
  ...
  if (
    key.startsWith("CODEXMANAGER_ALLOW_")
...
export function formatEnvOverrideDisplayValue(value: unknown) {
  const normalized = normalizeEnvTextValue(value);
  return normalized || "空";
}

export function formatEnvOverrideScopeLabel(scope: string | null | undefined) {
...
export function formatEnvOverrideApplyModeLabel(applyMode: string | null | undefined) {
  const normalized = String(applyMode || "").trim().toLowerCase();
  return normalized === "restart" ? "重启生效" : "热生效";
}

export function buildEnvOverrideHint(
  item: Pick<EnvOverrideCatalogItem, "key" | "name" | "scope" | "applyMode" | "defaultValue"> | null | undefined,
  ...
  parts.push(`作用域：${formatEnvOverrideScopeLabel(item.scope)}`);
  parts.push(formatEnvOverrideApplyModeLabel(item.applyMode));
  return parts.join("；");
}

export function filterEnvOverrideCatalog(catalog: EnvOverrideCatalogItem[], keyword: string) {
  ...
  return catalog.filter((item) => {
    const text = [
      item.key,
      buildEnvOverrideOptionLabel(item),
      buildEnvOverrideDescription(item),
      item.scope,
      item.applyMode,
    ]
...
export function normalizeEnvOverrideCatalogItem(value: unknown): EnvOverrideCatalogItem | null {
  if (!value || typeof value !== "object") {
    return null;
  }

  const record = value as Record<string, unknown>;
  const key = normalizeEnvKey(record.key);
  if (!key) {
    return null;
  }
  ...
  const name =
    String(
      ...
      || "",
    ).trim() || fallbackEnvLabel(key);
  const scope = String(record.scope || "service").trim().toLowerCase() || "service";
  const applyMode = String(record.applyMode || "runtime").trim().toLowerCase() || "runtime";
  const defaultValue = normalizeEnvTextValue(record.defaultValue);
  const description =
    String(record.description || "").trim()
    || buildEnvOverrideDescription({ key, name });
